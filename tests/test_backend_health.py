...
def test_market_data_quality_endpoint_returns_coverage_metrics():
    ...
    assert payload["metrics"]["active_catalog_verified_sets"] == 32
    assert payload["metrics"]["active_catalog_verified_pct"] == 64.0
    assert payload["metrics"]["active_catalog_evidence_started_sets"] == 50
    assert payload["metrics"]["active_catalog_blocked_sets"] == 18
...
def test_market_active_evidence_endpoint_returns_audit_log():
    ...
    payload = response.json()
    assert payload
    normalized_falcon = next(item for item in payload if item["set_id"] == "75192")
    verified_sets = [item for item in payload if item["ready_for_verified"] is True]
    assert normalized_falcon["verification_status"] == "verified"
    assert normalized_falcon["ready_for_verified"] is True
    assert len(verified_sets) == 32
