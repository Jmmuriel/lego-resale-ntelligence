...
def test_market_data_quality_endpoint_returns_coverage_metrics():
    ...
    assert payload["metrics"]["active_catalog_verified_sets"] == 47
    assert payload["metrics"]["active_catalog_verified_pct"] == 94.0
    assert payload["metrics"]["active_catalog_evidence_started_sets"] == 50
    assert payload["metrics"]["active_catalog_blocked_sets"] == 3
