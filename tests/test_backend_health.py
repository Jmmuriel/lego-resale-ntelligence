...
def test_market_data_quality_endpoint_returns_coverage_metrics():
    ...
    assert payload["metrics"]["active_catalog_verified_sets"] == 49
    assert payload["metrics"]["active_catalog_verified_pct"] == 98.0
    assert payload["metrics"]["active_catalog_evidence_started_sets"] == 50
    assert payload["metrics"]["active_catalog_blocked_sets"] == 1
